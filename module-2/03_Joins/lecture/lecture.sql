...
SELECT COUNT(city.state_abbreviation) AS num_of_cities, city.state_abbreviation FROM state
JOIN city ON state.state_abbreviation = city.state_abbreviation
WHERE census_region = 'Midwest'
...
SELECT state_name, MIN(date_established) AS oldest_park FROM state
